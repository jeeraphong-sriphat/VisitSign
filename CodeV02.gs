...
function saveSignatureImage_(base64Data, personName, dateStr) {
  ...
  // ใช้ URL รูปแบบ thumbnail ซึ่ง Google Sheets IMAGE() และ <img> tag โหลดได้เสถียรกว่ารูปแบบ uc?id=
...
function exportMonthlyReportPDF(yearMonth) {
  ...
  try {
    ...
    // เผื่อเวลาให้สูตร IMAGE() โหลดรูปจาก Drive ให้เสร็จก่อน export เป็น PDF
    // (ถ้าไม่รอ รูปที่เพิ่งใส่ใหม่อาจยังโหลดไม่ทันตอน export ทำให้ขึ้น #REF! หรือรูปว่าง)
